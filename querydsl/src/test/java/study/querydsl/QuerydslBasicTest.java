...
//        assertThat(loaded).as("페치 조인 적용").isTrue();
    ...
    public void basicCase() {
        List<String> result = queryFactory.select(member.age.when(10).then("열살")
                .when(20).then("스무살").otherwise("기타")).from(member).fetch();
    ...
    /**
     * 런타임 오류 Projections.fields(UserDto.class, member.username.as("name"), member.age, member.Id
     */
    @Test
...
//        em.clear();
    ...
    public void bulkDelete() {
        queryFactory.delete(member).where(member.age.gt(18));
    }
